...
The result `R` is the same as `Y` except that all character data within it is normalised as specified.
...
!!! note
    Because normalisation can change the length of character encodings, character data in `R` may be of a different shape and/or rank to that in `Y`.
