...
class LinkChecker:
    def __init__(self, base_url, max_concurrent=5, output_file='broken_links.txt'):
        ...
        self.broken_links = defaultdict(set)  # Map of broken link -> pages that reference it
    ...
    def extract_all_links(self, url, html):
        ...
        content = soup.find('article') or soup.find('main') or soup.find('div', class_='md-content')
        ...
        try:
            async with session.get(page_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status < 400:
                    ...
                    for link in links:
                        if link in self.checked_links and not self.checked_links[link][0]:
    ...
    async def spider(self):
        ...
        try:
            ...
            headers = {'User-Agent': 'Mozilla/5.0 (compatible; Dyalog Documentation Link Checker)'}
            ...
            async with aiohttp.ClientSession(connector=connector, headers=headers) as session:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Fetching navigation structure...", file=sys.stderr)
                try:
                    async with session.get(start_url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                        if response.status >= 400:
                            print(f"ERROR: Could not fetch home page: {response.status}", file=sys.stderr)
                            return
                        html = await response.text()
                except Exception as e:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Found {len(self.pages)} pages in navigation", file=sys.stderr)
                
                # Process all pages concurrently
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Processing pages and checking links...", file=sys.stderr)
                tasks = [self.process_page(session, page_url) for page_url in self.pages]
                ...
                for i in range(0, len(tasks), batch_size):
                    ...
                    # Progress update
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] Progress: {min(i+batch_size, len(tasks))}/{len(tasks)} pages | "
                          f"{len(self.checked_links)} links checked | "
                          f"{len(self.problematic_pages)} pages with issues | "
                          f"{len(self.broken_links)} broken links", file=sys.stderr)
        finally:
    ...
    def report(self):
        ...
        # Summary to stderr
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Link check complete!", file=sys.stderr)
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Problematic pages written to: {self.output_file}", file=sys.stderr)
...
def main():
    ...
    # Start message
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Starting link check for: {args.base_url}", file=sys.stderr)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Max concurrent requests: {args.max_concurrent}", file=sys.stderr)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Output file: {args.output}", file=sys.stderr)
