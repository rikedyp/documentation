...
The native file functions `⎕NCOPY`, `⎕NMOVE`, and `⎕NINFO`, support a variant option named `ProgressCallback` to enable progress callbacks.

### Overview
If this option is enabled, the system function invokes an APL callback function at various times whilst the file operation proceeds. A system object is used to communicate between the system function and the callback. The file operation has four distinct stages:
